(function () {
  ...
  document.addEventListener("WDB_INSTALL_SCRIPT", (e) => {
    ...
    sendToBackground(
      { action: "installScript", script: { name, content, language } },
      (res) => dispatch("WDB_INSTALL_SCRIPT_RESULT", res)
    );
  });
  ...
  document.addEventListener("WDB_UPDATE_SCRIPT", (e) => {
    ...
    sendToBackground(
      { action: "updateScript", script: { name, content, language } },
      (res) => dispatch("WDB_UPDATE_SCRIPT_RESULT", res)
    );
  });
  ...
  document.addEventListener("WDB_DELETE_SCRIPT", (e) => {
    ...
    sendToBackground(
      ...
      (res) => dispatch("WDB_DELETE_SCRIPT_RESULT", res)
    );
  });
  ...
  document.addEventListener("WDB_GET_SCRIPTS", () => {
    ...
    sendToBackground(
      { action: "getScripts" },
      (res) => dispatch("WDB_GET_SCRIPTS_RESULT", res)
    );
  });
  ...
  document.addEventListener("WDB_RUN_SCRIPT", (e) => {
    ...
    sendToBackground(
      ...
      (res) => {
        ...
        if (!script) {
          dispatch("WDB_RUN_SCRIPT_RESULT", { success: false, message: `Script "${name}" not found.` });
          return;
        }
        ...
        if (mode === "lua" && window.luaEditor) {
          ...
          dispatch("WDB_RUN_SCRIPT_RESULT", { success: true, message: `"${name}" loaded into Lua editor.` });
        } else if (mode === "js" && window.jsEditor) {
          window.jsEditor.setValue(script.content);
          dispatch("WDB_RUN_SCRIPT_RESULT", { success: true, message: `"${name}" loaded into JS editor.` });
        } else {
          dispatch("WDB_RUN_SCRIPT_RESULT", { success: false, message: "Bot editor not found. Is the bot running?" });
        }
      }
    );
  });

  console.log(`${BRIDGE_NAME} Ready — listening for: WDB_INSTALL_SCRIPT, WDB_UPDATE_SCRIPT, WDB_DELETE_SCRIPT, WDB_GET_SCRIPTS, WDB_RUN_SCRIPT`);
})();
