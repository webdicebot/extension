...
async function updateAlwaysActiveRules() {
  try {
    ...
    if (domains.length > 0) {
      ...
      console.log('[Web DiceBot] Always Active Rules updated for domains:', domains)
    }
  } catch (err) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // ── updateSettings: update API token, license, and API URL ────────
  if (request.action === 'updateSettings') {
    const { settings } = request
    const toSet = {}
    if (settings.auth_token !== undefined) toSet.auth_token = settings.auth_token
    if (settings.license !== undefined) toSet.license = settings.license
    if (settings.wdb_api !== undefined) toSet.wdb_api = settings.wdb_api

    chrome.storage.local.set(toSet, () => {
      sendResponse({ success: true, message: 'Settings updated from web app!' })
    })
    return true
  }
})
